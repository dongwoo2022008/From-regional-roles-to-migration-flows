"""
Follow-up 1: moving-window and three-period reconfiguration (strengthening RQ2 power)
(A) Annual pipeline intensity 2008-2025 (annual-modal roles, that year's flows) + OLS linear trend + region bootstrap CI.
(B) Three-period (2008-13/2014-19/2020-25, 6-year windows) period-modal reconfiguration: intensity, sign matches + per-channel monotone trend.
Pipeline intensity = mean of the predicted-direction E over the 13 pre-registered pairs.  SEED=20260716, B=1000.
"""
...
# ---------- (A) Annual intensity + trend ----------
Ay={y:build_A([y]) for y in range(2008,2026)}
annual={}  # (c,y)->role idx (that year's argmax)
for c in codes:
...
# region bootstrap: resample regions -> recompute intensity and slope over all years
...
print('=== (A) Annual pipeline intensity 2008-2025 ===')
print(' '.join(f'{y}:{v:+.1f}' for y,v in zip(years,inten_series)))
print(f'OLS trend slope {slope:+.3f}/yr (bootstrap 95% CI [{lo:+.3f},{hi:+.3f}], p={min(pslope,1.0):.4f})')
print(f'Predicted intensity 2008 {icpt:+.2f} → predicted intensity 2025 {icpt+slope*17:+.2f}')

# ---------- (B) Three 6-year periods ----------
PERIODS=[('2008-2013',range(2008,2014),(2008,2013)),
         ('2014-2019',range(2014,2020),(2014,2019)),
         ('2020-2025',range(2020,2026),(2020,2025))]
print('\n=== (B) Three-period (6-year windows) reconfiguration ===')
per_int={}; per_E={}
Ap={}
for nm,yrs,(w0,w1) in PERIODS:
    ...
    print(f'  {nm}: intensity {per_int[nm]:+.2f} | sign matches {sm}/13 | regions {len(idx)}')
# Per-channel three-period values + monotone trend
print('\n  Per-channel three-period E (monotone up/down marked):')
...
print(f'  Monotone-trend channels: {nmono}/13')
...
print('\nSaved: reconfig_trend.json, reconfig_trend.npz')
